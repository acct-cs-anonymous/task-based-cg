...
for k in {2..6}; do
    ...
    python -m scripts.generate_data  --split_strategy combination_$k --task_max_length $k --function_type diverse
done
...
for k in {1..6}; do
    ...
    python -m scripts.generate_data  --split_strategy combination_$k --task_max_length 7 --function_type diverse
done
...
# first run below commands to learn pair-wise composition equivalence scores among all functions in the diverse benchmark for K=6
...
for size in "${shared_equivalence_size[@]}"; do
    python -m scripts.generate_data --prompt_length fixed --split_strategy equivalence_6_576_$size --n_functions 6 --task_max_length 7 --n_alphabets 26 --seq_len 6 --function_type uniform 
done
...
for size in "${training_equivalence_size[@]}"; do
    python -m scripts.generate_data --prompt_length fixed --split_strategy uniequivalence_6_576_$size --n_functions 6 --task_max_length 7 --n_alphabets 26 --seq_len 6 --function_type uniform 
done
